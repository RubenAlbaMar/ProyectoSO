...
namespace ProjecteSO
{
    ...
    public partial class Wellcome : Form
    {
        ...
        private void buttonPEOPLECONNECTED_Click(object sender, EventArgs e)
        {
            string mensaje7;

            if (textBoxRANKING.Text != null)
            {
                mensaje7 = "7/" + textBoxRANKING.Text;
                byte[] msg7 = System.Text.Encoding.ASCII.GetBytes(mensaje7);
                server.Send(msg7);

                byte[] msg77 = new byte[1000];
                server.Receive(msg77);
                string respuesta77 = Encoding.ASCII.GetString(msg77).Split('\0')[0];
                string [] ListaConectados = respuesta77.Split('/');
                MessageBox.Show(ListaConectados[1]);
                MessageBox.Show(ListaConectados[2]);
                label26.Visible = true;
                L1.Visible = true;
                L2.Visible = true;
                L1.Text = ListaConectados[1];
                L2.Text = ListaConectados[2];
